...
function updateDateInputValue() {
  ...
  const selectedDate = new Date(year, month - 1, day); // Місяць у JavaScript починається з 0
...
dateInput.addEventListener('blur', updateDateInputValue);
setDatePeriod();
...
//     const TELEGRAM_BOT_TOKEN = '6348717892:AAFL-zMwB_fGctvIv7aftpcQh8z7OtEEoyQ';
//     const TELEGRAM_CHAT_ID = '-1002025349539';
//     const API = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`;

//     const formBtn = document.querySelector('.btn_send');
        
//     const { name, number, cityStart, cityEnd, date, persone } = Object.fromEntries(new FormData(form).entries());
    
//     const text = `Заявка від:\n${name}\nТелефон: ${number}\nМісто відправки: ${cityStart}\nМісто прибуття: ${cityEnd}\nДата поїздки: ${date}\nК-сть: ${persone}\n `;
...
form.addEventListener('submit', function (e) {
  e.preventDefault();
  sendEmailTelegram(form);

});

async function sendEmailTelegram(form) {
  const formData = new FormData(form);
  const { name, number, cityStart, cityEnd, date, persone } = formData;
  const text = `${name}\nТелефон: ${number}\nМісто відправки: ${cityStart}\nМісто прибуття: ${cityEnd}\nДата поїздки: ${date}\nК-сть: ${persone}\n `;
